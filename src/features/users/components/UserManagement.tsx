...
export function UserManagement({ hasPermission }: UserManagementProps) {
  ...
  const handleSaveUser = async (userData: any) => {
    ...
    try {
      ...
      // Step 2: Create Empleado or Cliente record depending on role (with auto-generated document number)
      const roleName = (selectedRole?.nombre || '').toLowerCase();
      const generate10DigitNumber = () => {
        return Math.floor(1000000000 + Math.random() * 9000000000).toString();
      };
      const autoGeneratedDoc = generate10DigitNumber();

      if (roleName === 'cliente') {
        await apiClient.post('/api/Clientes', {
          documentoCliente: autoGeneratedDoc,
          usuarioId: usuarioId,
          tipoDocumento: 'CC',
          nombre: userData.nombre,
          telefono: userData.phone,
        ...
        await apiClient.post('/api/Empleados', {
          documentoEmpleado: autoGeneratedDoc,
          usuarioId: usuarioId,
          tipoDocumento: 'CC',
          nombre: userData.nombre,
          telefono: userData.phone,
...
function UserModal({ user, onClose, onSave, roles }: { user: any; onClose: () => void; onSave: (data: any) => void; roles: RolListDto[] }) {
      ...
      case 'documento': {
        ...
        if (value.trim()) {
          if (!/^[a-zA-Z0-9]+$/.test(value.trim()))
            return 'El documento de usuario solo debe contener letras y números';
          if (value.trim().length < 5 || value.trim().length > 20) return 'El documento de usuario debe tener entre 5 y 20 caracteres';
        }
        return '';
      }
      case 'phone':
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fieldsToValidate = !user
      ? ['nombre', 'email', 'documento', 'phone', 'direccion', 'password']
